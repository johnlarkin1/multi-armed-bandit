...
from flaky_load_balancer.config import LB_CONFIG_TARGET, LB_STRATEGY
...
async def lifespan(app: FastAPI):
    ...
    init_strategy(LB_STRATEGY, config_target=LB_CONFIG_TARGET)
    run_id = get_csv_logger().start_new_run(LB_STRATEGY, session_id=LB_SESSION_ID, config_target=LB_CONFIG_TARGET)
    get_metrics_collector().reset()  # Reset metrics
    print(
        f"Started new run: {run_id} [config={LB_CONFIG_TARGET}]"
        + (f" (session: {LB_SESSION_ID})" if LB_SESSION_ID else "")
    )
    yield
